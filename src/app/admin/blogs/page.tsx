...
export default function AdminBlogsPage() {
  ...
  return <div style={{ maxWidth: 1400, margin: "0 auto" }}><Card title="Blogs" extra={<Button type="primary" onClick={() => { setEditing(null); setImage(null); form.resetFields(); setOpen(true); }}>Add Blog</Button>}><Table rowKey="_id" columns={columns} dataSource={rows} loading={query.isLoading} /></Card><Modal open={open} onCancel={() => setOpen(false)} onOk={() => form.submit()} confirmLoading={saveMutation.isPending} title={editing ? "Edit Blog" : "Add Blog"} width={1120}><Form form={form} layout="vertical" initialValues={{ isPublished: true }} onFinish={(values) => saveMutation.mutate({ ...values, imageUrl: image?.url || values.imageUrl, imagePublicId: image?.publicId || values.imagePublicId })}><Form.Item label="Cover image"><input type="file" accept="image/*" onChange={(event) => handleImage(event.target.files?.[0])} />{image?.url && <img src={image.url} alt="Preview" style={{ display: "block", width: 180, marginTop: 12, borderRadius: 12 }} />}</Form.Item><Form.Item name="imageUrl" hidden><Input /></Form.Item><Form.Item name="imagePublicId" hidden><Input /></Form.Item><Form.Item label="Title" name="title" rules={[{ required: true }]}><Input /></Form.Item><Form.Item label="Slug" name="slug"><Input /></Form.Item><Form.Item label="Excerpt" name="summary"><Input.TextArea rows={2} /></Form.Item><Form.Item label="Description" name="body" rules={[{ required: true }]}><CkeditorBlogEditor /></Form.Item><Form.Item label="Author" name="authorName"><Input placeholder="Inner Beast" /></Form.Item><Form.Item label="Published" name="isPublished" valuePropName="checked"><Switch /></Form.Item></Form></Modal></div>;
}
